...
2. **存储方式**：未声明 → 默认 `local`，告知用户。`feishu` 需验证 CLI 可用（`which feishu`）。
...
**输出**：`docs/requirements/<feature-slug>-clarification.md`
...
**输出**：`docs/requirements/<feature-slug>.md`（local）或飞书文档（feishu）

确认后进入 Step 3。

### Step 3：生成总体计划

> **L3 加载**：`references/overall-plan-template.md`
...
Read `references/overall-plan-template.md`，按垂直切片原则划分阶段：

- **Phase 1 (Tracer Bullet)**：最简端到端流程，覆盖所有技术层但只实现 Happy Path
- **Phase 2**：核心业务逻辑 + 错误处理 + 输入校验
- **Phase 3**：边界情况 + 外部集成 + 异步处理
- **Phase N (最后)**：生产化 — 监控、文档、回滚方案

#### 3.3 生成总体计划文档

按模板输出，包含：项目概述、阶段划分表（目标/任务数/完成标准/状态）、依赖关系图、里程碑、风险。

为每个阶段的宏观任务创建 TODO（TaskCreate）。

**输出**：`docs/requirements/<feature-slug>-overall-plan.md`

完成后告知用户阶段数量，进入 Step 4 为 Phase 1 生成详细计划。

### Step 4：生成详细计划

> **L3 加载**：`references/plan-template.md`、项目架构约束文档
...
按计划模板中的原则（垂直切片 / 1-3 天规则 / 独立可测 / 依赖清晰）拆分阶段任务为叶子任务。

任务 ID 格式：`<SLUG>-P<phase>-<seq>`（如 `LOGIN-P1-01`）

每个任务包含：描述、验收标准（checkbox）、验证方式、依赖、预估涉及文件、规模（S/M/L）、逻辑概要（伪代码，可选）。

规模判断：S=1-2 文件、M=3-5 文件、L=5+ 文件需进一步拆分。

#### 4.3 生成详细计划文档

按 `plan-template.md` 结构输出，包含：阶段目标、架构约束、任务列表、依赖关系图、检查点、测试策略、不在范围的事项。

为每个叶子任务创建 TODO（TaskCreate）。

**输出**：`docs/requirements/<feature-slug>-detailed-plan-p<n>.md`

完成后告知用户任务数、依赖关系和并行可能性，进入 Step 5。

### Step 5：询问派发实施

> **L3 加载**：`references/dispatch-strategies.md`
...
派发时按 `dispatch-strategies.md` 第五章完整 Prompt 模板构造子 agent prompt，**必须注入以下开发规范**：
