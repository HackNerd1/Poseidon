...
Date: `2026-06-30`
Scope: `杭州无限画布/Canvas 相关工作机会分析`
Owner: `job:job-opportunity-analyzer`
Status: `open`

## Summary

本次在分析“杭州无限画布/Canvas 相关岗位”时，成功确认了少量高相关公司与官方招聘入口，但在“岗位级事实提取”阶段暴露出一组稳定问题：
...
Problem:
- 用 `杭州 + 画布/白板/编辑器 + 招聘` 这类搜索词时，搜索结果容易漂移
- Bing 结果中多次出现和岗位无关的杭州旅游、百科、城市介绍页面
...
- 很多官方站可以确认公司存在、产品相关
- 但难以确认具体团队是否在杭州招人
- 特别是飞书、钉钉这类大平台，杭州可能只是公司维度布局，不代表目标团队一定在杭州
...
- 对“杭州 + Canvas 相关方向”，能产出至少 3 条岗位级机会
- 每条机会至少包含：
  - `company`
  - `job_title`
  - `job_url`
